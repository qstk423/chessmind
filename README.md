...
- **中国象棋**：可选 [Pikafish](https://github.com/official-pikafish/Pikafish) 走子（未安装则内建搜索 + 开局库）+ 启发式 Council；**人机 / 人人 / AI vs AI**（对手可选引擎 / GLM / 千问）；时限 15/10/5 分钟（超时判负）；悬子绿圈 / 己方被吃红圈；将军提示、赛后复盘折线（上红优、下黑优）
...
- 中国象棋：Pikafish 可选强力引擎、难度档、禁止无意义原路退回、悬子/被吃标注、对局时钟、将军提示、赛后复盘曲线分色
- 名局 / 残局 / 战术学习与残局闯关
- WebSocket 联机房间（同网演示级）
- 国际象棋：拍照识谱、对局历史、PGN、路演 Demo；人机可选手动对手（GLM / 千问 / 引擎）
...
- 中国象棋：**时限 15 / 10 / 5 分钟**（默认 15，超时判负）；可吃未保护敌子 **绿圈**、己方悬子 **红圈**
...
- **象棋：AI vs AI、时限超时判负、悬子绿/红标注、防原路退回、默认引擎强档**
